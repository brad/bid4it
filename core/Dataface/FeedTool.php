...
class Dataface_FeedTool {
	...
	function getRelatedTitle($record, $relationshipName){
		return "Related records of ".$record->getTitle();
	...
	function getRelatedLink($record, $relationshipName){
		return $record->getURL('-relationship=$relationshipName&-action=related_records_list');
	}
		...
		if ( isset($query['-relationship']) ){
			...
			if ( count($rrecords) > 0 ){
				...
				foreach ($rrecords as $rrec){
					...
					if ( $dfRecord->checkPermission('view') ){
						$records[] =& $dfRecord;
					}
					unset($dfRecord);
					unset($rrec);
