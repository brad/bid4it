...
class Dataface_IO {
	...
	function _insert(&$record, $tablename=null, $secure=false){
		...
		if ( !$res || PEAR::isError($res)){
			...
			trigger_error(
				...
				.mysql_error($s->db).": SQL: $sql".Dataface_Error::printStackTrace(), E_USER_ERROR);
		}
	...
	function performSQL($sql){
		...
		while (count($queue) > 0 and $skips < count($queue) ){
	 ...
	 * </code>
	 */
	function &getByID($uri, $filter=null){
