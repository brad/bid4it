...
class Dataface_Index {
	...
	function indexFoundRecords($query, $lang='*'){
		for ( $start = 0; $start >= 0; $start +=100 ){
			...
			$records = df_get_records_array($query['-table'], $query, $start, 100);
	 ...
	 * @returns array
	 */
	function find($query, $returnMetadata=false){
		$select = "select record_id,`table`,record_url,record_title,record_description,`lang`,match(searchable_text) against ('".addslashes($query['-search'])."') as `relevance`";
		$sql = "
			
			from dataface__index
			where match(searchable_text)
