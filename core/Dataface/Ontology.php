...
class Dataface_Ontology {
	...
	function &newOntology($type, $tablename){
		$ontologies =& Dataface_Ontology::getRegisteredOntologies();
	...
	function registerType($type, $path, $class){
		$ontologies =& Dataface_Ontology::getRegisteredOntologies();
		$ontologies[$type] = array('type'=>$type, 'path'=>$path, 'class'=>$class);
		return true;
	}
	
	function &getRegisteredOntologies(){
		static $ontologies = 0;
