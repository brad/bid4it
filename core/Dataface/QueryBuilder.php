...
;
	...
	function wc($tablename, $colname){
		if ( in_array($this->action, array('select','delete')) ){
	...
	function _fieldWhereClause(&$field, $value, &$use_where, $tableAlias=null){
		...
		foreach ($words as $value){
			...
			foreach ($factors as $value){
				...
				else if ( $this->_exactMatches || eregi( 'int', $field['Type']) || $exact ){
					$oldval = $value;
					if ( eregi('char', $field['Type']) ) $oper = 'LIKE';
					else $oper = '=';
					$value = $this->prepareValue( $key, $table->parse($key, $value), true);
					if (  strlen($oldval) > 0 ){
	...
	function _where($query=array(), $merge=true){
		...
		$charFields = $this->_table->getCharFields(true);
		...
		foreach ( $rquery as $rname=>$rfields){
			...
			foreach ( $rfields as $rfield ){
				...
				$pairs[] = $this->_fieldWhereClause($rfieldDef, $q, $use_where, $r->getTableAlias($rfieldDef['tablename']));
				unset($rfieldDef);
	...
	function _from($tablename=null){
		...
		if ( isset($parent) ){
			...
			$pfrom = $qb2->_from();
			$as_pos = ( ( strpos(strtolower($pfrom), ' as ') !== false ) ? (strlen($pfrom) - strpos(strrev(strtolower($pfrom)), ' sa ' )) : false);
			if ( $as_pos !== false ){
	 ...
	 */
	function _select($columns='', $query=array(), $preview=false){
		$app =& Dataface_Application::getInstance();
		$query = array_merge( $this->_query, $query);
		foreach ($query as $key=>$value) {
			if ( $value === null ){
		...
		foreach ($this->_fields as $key=>$field){
			...
			if ( $preview and $this->_table->isText($field['name']) and !$this->_table->isXML($field['name'])) 
				$select .= "SUBSTRING(`{$this->_tablename}`.`$key`, 1, 255) as `$key`,";
