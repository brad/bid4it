...
class Dataface_Relationship {
	 ...
	 * @returns array
	 */
	function &fields($includeAll=false){
		if ( !$includeAll ){
			...
			if ( !isset($this->_cache[__FUNCTION__]) ){
				...
				foreach ( array_keys($tables) as $i ){
					foreach ( array_keys($tables[$i]->fields(false,true)) as $fld ){
						$out[] = $tables[$i]->tablename.'.'.$fld;
					}
				}
	 ...
	 *
	 */
	function hasField($fieldname, $checkAll=false){
		if ( strpos($fieldname,'.') === false ){
			return in_array($fieldname, $this->_schema['short_columns'] );
		} else {
			return in_array( $fieldname, $this->fields($checkAll) );
		}
	}
	...
	function supportsAddNew(){
		if ( isset( $this->_schema['actions']['addnew'] ) and !$this->_schema['actions']['addnew'] ) return false;
		return true;
	}
	...
	function supportsAddExisting(){
		if ( isset( $this->_schema['actions']['addexisting'] ) and !$this->_schema['actions']['addexisting']  ) return false;
		return true;
	}
	...
	function getDomainSQL(){
		...
		if ( !isset( $relationship['domain_sql'] ) ){
			...
			//if ( count($domain_tables) > 1 ){
				...
				$high = null;
				$high_score = 0;
				foreach ( $relationship['selected_tables'] as $dt ){
			...
			if ( count($domain_tables) !== 1 ){
				...
				//trigger_error("Problem here: ".implode(',',$domain_tables), E_USER_ERROR);
			...
			//$dtablenames = array();
			//foreach ($this->getDestinationTables() as $dtable){
			//	$dtablenames[] = $dtable->tablename;
			//}
			//if ( !in_array($relationship['domain_table'], $dtablenames) ){
			//	$desttables =& $this->getDestinationTables();
			//	$relationship['domain_table'] = $destablenames[0];
			//}
	...
	function getDomainTable(){
		...
		return $this->_schema['domain_table'];
	}
	...
	function getForeignKeyValues($values = null, $sql = null, $record = null){
		...
		foreach ( $labels as $field_name=>$label ){
			if ( !isset( $vals[$label] ) ){
				...
				if ( $field['Extra'] == 'auto_increment' ){
					...
					$vals[$label] = "__".$table."__auto_increment__";
				}
				unset($field);
			}
		}
		...
		foreach ( $labels as $field_name=>$label){
			...
			if ( !isset( $table_cols[$table] ) ) $table_cols[$table] = array();
 			$table_cols[$table][$field] = ( $record !== null  and !preg_match('/(blob|binary)/', strtolower($fieldArr['Type'])) ) ? $record->parseString(@$vals[$label]) : @$vals[$label];
			unset($fieldArr);
		}
	...
	function getDistance($table){
		...
		foreach ( $fkeys[$table] as $key=>$value ){
			if ( strlen($value) > 0 and $value{0} == '$' ) return 1;
