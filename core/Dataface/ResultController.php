...
class Dataface_ResultController {
   ...
   function &getRecords(){
	   if ( !isset( $this->_records) ){
		   ...
		   $this->_displayedRecords = mysql_num_rows($res);
   ...
   function getDisplayedRecords(){
   	...
   	function listHtml($prefix=''){
   		...
   		$out[] = '<li class="results-per-page"> Showing <input type="text" value="'.$limit.'" onchange="window.location = \''.$urlprefix.'&-'.$prefix.'limit=\'+this.value" size="3"/>'.df_translate('scripts.GLOBAL.MESSAGE_RESULTS_PER_PAGE','Results per page');
   		$out[] = '</ul>';
