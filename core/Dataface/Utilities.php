...
class Dataface_Utilities {
	 ...
	 */
	function groupBy($fieldname, $records, $order=array(), $titles=array()){
		import( 'PEAR.php');
		if (!is_array($records) ){
	 ...
	 */
	function query2html($query, $keyFilter=array()){
	 ...
	 * @return void
	 */
	function flattenQuery($in,&$out, $path=array()){
		$origPath = $path;
		if ( !empty($path) ){
	 ...
	 * @return void
	 */
	function fireEvent($name, $params=array()){
	 ...
	 */
	function redirect($msg=null, $error=null){
		$app =& Dataface_Application::getInstance();
		$query =& $app->getQuery();
		$prefix = @$query['--prefix'];
