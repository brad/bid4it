...
function df_init($site_path, $dataface_url, $conf=null){
	if ( !defined('DATAFACE_SHOW_NOTICES') or !DATAFACE_SHOW_NOTICES){
...
function &df_get_related_records($query=array()){
	...
	$start = isset($_REQUEST['-related:start']) ? $_REQUEST['-related:start'] : 0;
 	$limit = isset($_REQUEST['-related:limit']) ? $_REQUEST['-related:limit'] : 30;
...
function df_query($sql, $lang=null, $as_array=false){
	import('Dataface/DB.php');
	$db =& Dataface_DB::getInstance();
	return $db->query($sql,null,$lang,$as_array);
}
...
function df_translate($id, $default=null, $params=array()){
	return Dataface_LanguageTool::translate($id,$default,$params);
}
