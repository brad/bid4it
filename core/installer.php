...
class Dataface_Installer {
	...
	function mainMenu(){
		include('install/mainMenu.inc.php');
	}
	...
	function archive2app(){
		...
		$form->addRule('mysql_user', 'Please enter a mysql username for the application to connect as.', 'required',null,'client');
		$form->addRule('install_type', 'Please select an installation type.', 'required', null, 'client');
		...
		$out = ob_get_contents();
		ob_end_clean();
		include 'install/archive2app.inc.php';
	}
	...
	function archive2app__process($values){
		...
		if ( !$res ){
			...
			if ( !$res ){
				return PEAR::raiseError("Cannot create database '$dbname'");
			}
			$res = mysql_select_db($dbname);
			if ( !$res ){
				return PEAR::raiseError("Cannot select database $dbname.");
			}
		}
		...
		switch ($values['install_type'] ){
			case 'ftp_install':
				...
				include 'install/archive2app-results.inc.php';
				exit;
	...
	function db2app(){
		...
		$out = ob_get_contents();
		ob_end_clean();
		include 'install/db2app.inc.php';
	}
	...
	function db2app__process($values){
		...
		//echo $tarpath;
		$archive = new Archive_Tar($tarpath,'gz');
...
'
	...
	function test_db_access($dbname, $username, $password){
		...
		if ( !$db ){
			return PEAR::raiseError("Could not connect to the MySQL server as $username.");
		}
	...
	function test_ftp_access($host, $path, $user, $password, $ssl=false){
		...
		if ( !$conn ) return PEAR::raiseError("Could not connect to FTP");
		
		$res = @ftp_login($conn, $user, $password);
		if ( !$res ) return PEAR::raiseError("Cannot login to FTP server with the provided username ($user) and password");
		
		$res = @ftp_chdir($conn, $path);
		if ( !$res ){
			return PEAR::raiseError("The directory $path on the server $host does not exist.");
		}
		return true;
	}
...
function db(){
	...
	if ( $db == -1 ){
		if (!@$_SERVER['PHP_AUTH_USER'] || !$_COOKIE['logged_in'] ){
			Dataface_Installer::authenticate();
		}
