...
class HTML_QuickForm_Renderer_Array extends HTML_QuickForm_Renderer
{
    ...
    */
    ...
    {
    	if ( $this->_assoc ){
    		 $this->_ary['sections'][$header->getName()] = array(
				'header' => $header->toHtml(),
				'name'   => $header->getName()
			);
			$this->_currentSection = $header->getName();
		} else {
    ...
    */
    function _storeArray($elAry)
    {
        ...
        if (is_array($this->_currentGroup) && ('group' != $elAry['type'])) {
        	...
        	if ( $this->_assoc ){
        		$this->_ary['sections'][$this->_currentSection]['elements'][$elAry['name']] = $elAry;
        	} else {
            	$this->_ary['sections'][$this->_currentSection]['elements'][] = $elAry;
            }
        } else {
        	if ( $this->_assoc ){
				$this->_ary['elements'][$elAry['name']] = $elAry;
			} else {
