...
// {{{ function SQL_Parser($string = null)
    function SQL_Parser($string = null, $dialect = "ANSI", $useCache = true) {
...
// }}}
    ...
    function getTok() {
        ...
        if ( isset( $this->functions[$this->token]) and
       ...
       if ($this->token == 'ident' || $this->isFunc() || $this->token == '*') {
       		while ($this->token != 'from') {
            	...
            	if ($this->token == 'ident' || $this->token == '*' ) {
					...
					$tree['columns'][] = array('type'=>$colType, 'table'=>$columnTable2, 'value'=>$columnName2, 'alias'=>$columnAlias);
					...
					$tree['columns'][] = array('type'=>'func', 'table'=>'', 'value'=>$result, 'alias'=> $columnAlias);
        ...
        while ($this->token == 'ident' or $this->token == '(') {
        	if ( $this->token == 'ident' ){
        		$tableType = 'ident';
        		$tree['table_names'][] = $tableName = $this->tokText;
        	} else {
        		 //must be a subselect.
				$this->getTok();
				if ( $this->token != 'select' ){
